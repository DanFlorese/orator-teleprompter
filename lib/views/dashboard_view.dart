...
class _DashboardViewState extends State<DashboardView> {
  ...
  // --- NUEVA FUNCIÓN: ELIMINAR DE SUPABASE ---
  Future<void> _deleteScript(String id) async {
    try {
      await Supabase.instance.client.from('scripts').delete().eq('id', id);
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text('Script deleted'), backgroundColor: Colors.redAccent),
        );
      }
    } catch (e) {
    ...
    return Scaffold(
      ...
      body: StreamBuilder<List<Map<String, dynamic>>>(
        ...
        builder: (context, snapshot) {
          ...
          if (snapshot.connectionState == ConnectionState.waiting) return const Center(child: CircularProgressIndicator(color: redOrator));
                ...
                child: Container(
                  ...
                  child: ListTile(
                    ...
                    subtitle: Padding(
                      ...
                      child: Text('$readTime min read • Swipe to delete', style: const TextStyle(color: Colors.white38, fontSize: 12)),
                    ),
                    trailing: InkWell(
                      onTap: () {
                        Navigator.push(context, MaterialPageRoute(builder: (context) => CameraView(scriptTitle: title, scriptContent: content)));
                      },
                      child: Container(
                        padding: const EdgeInsets.all(8),
                        decoration: BoxDecoration(color: redOrator.withValues(alpha: 0.1), shape: BoxShape.circle),
                        child: const Icon(Icons.videocam_rounded, color: redOrator, size: 28),
                      ),
                    ),
                    onTap: () async {
                      await Navigator.push(context, MaterialPageRoute(builder: (context) => ScriptEditorView(script: script)));
                      _refreshScripts();
                    },
                  ),
                ),
      ...
      floatingActionButton: FloatingActionButton.extended(
        ...
        label: const Text('NEW SCRIPT', style: TextStyle(fontWeight: FontWeight.bold, color: Colors.white)),
        onPressed: () async {
          await Navigator.push(context, MaterialPageRoute(builder: (context) => const ScriptEditorView()));
          _refreshScripts();
        },
      ),
    );
  ...
  Widget _buildEmptyState(BuildContext context) {
    return Center(
      child: Column(
        ...
        children: [
          Icon(Icons.auto_stories_outlined, size: 100, color: Colors.white.withValues(alpha: 0.1)),
          const SizedBox(height: 20),
          const Text('YOUR STAGE IS READY', style: TextStyle(color: Colors.white, fontSize: 20, fontWeight: FontWeight.bold)),
